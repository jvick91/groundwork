...
**Active task:** TASK-014 (auth middleware chain). TASK-014A complete; TASK-014 main (JWT middleware) is next.
**Branch:** task-014-auth-middleware
**Last architectural change:** 2026-05-27 — ADR-010 ratified (Accepted). SPEC-007 §3 updated to reflect Auth0 Organizations (`org_id` JWT claim, org-switch as fresh login, zero-staleness permission cache via `permissions_version`). ADR-008 marked Superseded by ADR-010. The auth chain contract is now fully specified; TASK-014 main may begin.
