...
- Cosign_required setter: The `cosign_required` boolean is `false` by default at note creation. The author (or any user with `notes.write` on the draft) may set it to `true` via POST or PATCH while the note is in `draft` status. Once the note transitions out of `draft` (at first sign), `cosign_required` becomes immutable — subsequent PATCH attempts that would change it return HTTP 422. Automatic derivation from author role (therapist-without-supervisor-privileges → true) or from an organization-level policy toggle is explicitly out of scope for MVP; it is a post-MVP enhancement.
