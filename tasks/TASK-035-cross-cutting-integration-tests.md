...
- [ ] `test_cross_cutting/test_permission_enforcement.py`: introspect the registered FastAPI router tree, enumerate every non-exempt route (excluding `/health*` and `/auth/me`), and assert each carries a `require_permission(...)` dependency per SPEC-002 §4 BR-06; failing the sweep names the unguarded route
